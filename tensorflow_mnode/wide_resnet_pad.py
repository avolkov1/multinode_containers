...
def WideResidualNetwork(depth=28, width=8, dropout_rate=0.0,
    ...
    xx = ZeroPadding2D(2)(img_input)

    # x = __create_wide_residual_network(
    #     classes, img_input, include_top, depth, width,
    #     dropout_rate, activation)

    x = __create_wide_residual_network(classes, xx, include_top, depth, width,
                                       dropout_rate, activation)
...
def __conv2_block(input_, k=1, dropout=0.0):
    init = input_
    ...
    x = Conv2D(16 * k, (3, 3), padding='same')(input_)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
...
def __conv3_block(input_, k=1, dropout=0.0):
    init = input_
    ...
    x = Conv2D(32 * k, (3, 3), padding='same')(input_)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
...
def ___conv4_block(input_, k=1, dropout=0.0):
    init = input_
    ...
    x = Conv2D(64 * k, (3, 3), padding='same')(input_)
    x = BatchNormalization(axis=channel_axis)(x)
    x = Activation('relu')(x)
...
def __create_wide_residual_network(
        nb_classes, img_input, include_top, depth=28,
