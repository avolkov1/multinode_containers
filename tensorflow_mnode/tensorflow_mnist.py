...
from dataset_mnist import train
...
MNIST_DATADIR = 'MNIST-data'


def get_data_mnist(batch_size):
    '''Load mnist data via TF Dataset API'''
    shuffle_buffer = 1000

    train_dset = train(MNIST_DATADIR)
    train_dset = train_dset.shuffle(shuffle_buffer).repeat().batch(batch_size)
    iterator = train_dset.prefetch(500).make_one_shot_iterator()
