...
if args.cuda:
    ...
    torch.cuda.set_device(hvd.local_rank())  # @UndefinedVariable
    torch.cuda.manual_seed(args.seed)  # @UndefinedVariable


def print_in_order(msg, comm=hvd):
    ...
    for irank in range(comm.size()):
        ...
        comm.allreduce(torch.tensor(0), name='barrier')
        # comm.allreduce(torch.FloatTensor([0]), name='barrier')
        if comm.rank() == irank:
            print(msg)


msg = 'WORKER Rank {}; GPU {}'.format(hvd.rank(), hvd.local_rank())
print_in_order(msg)

kwargs = {'num_workers': 1, 'pin_memory': True} if args.cuda else {}

# DATADIR = 'data-%d' % hvd.rank()
DATADIR = 'data'
...
def metric_average(val, name):
    tensor = torch.FloatTensor([val])  # @UndefinedVariable
